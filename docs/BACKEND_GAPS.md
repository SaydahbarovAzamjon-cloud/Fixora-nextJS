...
| GAP-098 | **Saved technicians list (client My Page)** | `getUserLikedTechnicians(input)` — list technicians the auth user liked via `likeTargetUser` | Owner My Page Saved Technicians tab; `likeTargetUser` mutation exists but no list query | Empty state on `/client/my-page?tab=savedTechnicians`; count stays 0 | `MISSING` | `libs/components/mypage/fixora/SavedTechniciansTab.tsx`, `pages/client/my-page/index.tsx` |
