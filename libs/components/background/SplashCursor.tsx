...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function hexToRGB(hex: string, intensity = config.colorIntensity): ColorRGB {
      let val = hex.replace('#', '');
    ...
    function generateColor(): ColorRGB {
      if (config.colorPalette.length > 0) {
        const hex = config.colorPalette[Math.floor(Math.random() * config.colorPalette.length)];
        return hexToRGB(hex);
      }
      ...
      c.r *= config.colorIntensity;
      c.g *= config.colorIntensity;
      c.b *= config.colorIntensity;
      return c;
    }
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixora-splash-bg__canvas"
      aria-hidden="true"
    />
  );
}
